...
public abstract class AbstractNonCHBidirAlgo extends AbstractBidirAlgo implements EdgeToEdgeRoutingAlgorithm {
    ...
    public AbstractNonCHBidirAlgo(Graph graph, Weighting weighting, TraversalMode tMode, PathCache pathCache) {
        super(tMode,pathCache,graph);
    ...
    protected void fillEdgesFromUsingFilter(EdgeFilter edgeFilter) {
        ...
        finishedFrom = fillEdgesFrom()==null;
    ...
    protected void fillEdgesToUsingFilter(EdgeFilter edgeFilter) {
        additionalEdgeFilter = edgeFilter;
        finishedTo = fillEdgesTo()==null;
        additionalEdgeFilter = null;
    }

    @Override
    SPTEntry fillEdgesFrom() {
        while (true) {
...
//        }
    ...
    public int getTraversalId(SPTEntry entry){
        if (this.traversalMode.isEdgeBased()) {
            return entry.edge;
        } else {
            return entry.adjNode;
        }
    }
...
//        if(cacheTree.nodeMap.containsKey(getTraversalId(currEdge))){
//            //load the cache into prioQueue
//            SPTEntryCache cacheNode = cacheTree.nodeMap.get(getTraversalId(currEdge));
//            for(SPTEntryCache child:cacheNode.childs){
//                if(child.node.isDeleted()){
//                    continue;
//                }
//                prioQueue.add(child.node);
//                bestWeightMap.put(getTraversalId(child.node),child.node);
//            }
//        }
        ...
        while (iter.next()) {
            ...
            if (entry == null) {
                ...
                bestWeightMap.put(traversalId, entry);
                prioQueue.add(entry);
//                cacheTree.addNode(entry,traversalId,currEdge,getTraversalId(currEdge),GHUtility.calcWeightWithTurnWeight(weighting, iter, reverse, currEdge.edge));
    ...
    protected Path extractPath2(Path pathCache,SPTEntry fromCache,SPTEntry toCache) {
        return createPathExtractor(graph, weighting).extract2(fromCache,toCache, bestWeight,pathCache);
    }
